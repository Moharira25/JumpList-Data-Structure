...
private:
    ...
    //Takes a gap node with segment size > 5 and split it
    //to two segments according to its length, if it is even it
    //will be split evenly, and if its odd the first segemnt will be 
    // one node larger.
    void splitSegment(Node* node, int gap);
